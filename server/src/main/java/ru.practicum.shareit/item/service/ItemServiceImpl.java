...
public class ItemServiceImpl implements ItemService {
    ...
    public ItemDto createItem(Long userId, NewItemRequest itemRequest) {
        ...
        ItemRequest linkedRequest = null;
        if (itemRequest.getRequestId() != null) {
            linkedRequest = itemRequestRepository.findById(itemRequest.getRequestId())
                    .orElseThrow(() -> new NotFoundException("Запрос с ID " + itemRequest.getRequestId() +
                            " не найден"));
        }

        Item item = ItemMapper.mapToItem(user, itemRequest, linkedRequest);
